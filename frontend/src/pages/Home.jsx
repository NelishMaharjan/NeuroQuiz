...
const Home = () => {
  ...
  // Added 'path' property to each category
  const categories = [
    { name: "Science", icon: "🧬", count: "120+ Questions", color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20", path: "/science" },
    { name: "Technology", icon: "💻", count: "85+ Questions", color: "bg-blue-500/10 text-blue-600 border-blue-500/20", path: "/technology" },
    { name: "History", icon: "📜", count: "60+ Questions", color: "bg-amber-500/10 text-amber-600 border-amber-500/20", path: "/history" },
    { name: "Geography", icon: "🌍", count: "45+ Questions", color: "bg-indigo-500/10 text-indigo-600 border-indigo-500/20", path: "/geography" },
  ];
        ...
        <div className="max-w-6xl mx-auto px-6 mt-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {categories.map((cat, i) => (
              <div 
                key={i} 
                onClick={() => navigate(cat.path)} // Logic added here
                className="group relative bg-white border border-slate-200/60 p-6 rounded-[2rem] cursor-pointer hover:border-slate-300 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-300 active:scale-[0.97]"
              >
