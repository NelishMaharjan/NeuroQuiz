    ...
    <div className="min-h-screen bg-[#fafafa] font-sans selection:bg-slate-900 selection:text-white">
      ...
      <div className="max-w-7xl mx-auto pt-32 pb-24 px-6 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <div className="bg-white border border-slate-200/60 py-4 px-6 rounded-[2rem] shadow-sm flex items-center gap-4">
            ...
            <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Academy Member</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm group hover:border-slate-900 transition-all duration-500">
        ...
        <div className="bg-white border border-slate-200/60 rounded-[3rem] shadow-2xl shadow-slate-200/40 overflow-hidden">
          <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between">
            <h2 className="text-lg font-black text-slate-900 tracking-tight">Recent Sessions</h2>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Module Category</th>
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Precision Score</th>
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Completion Date</th>
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Evaluation</th>
                </tr>
              </thead>
                ...
                {results.map((res) => {
                  ...
                  return (
                    <tr key={res.id} className="group hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="px-10 py-6">
                        <p className="text-xs font-bold text-slate-600">{new Date(res.createdAt).toLocaleDateString()}</p>
                      </td>
                ...
                {results.length === 0 && (
                  <tr>
                    <td colSpan="4" className="px-10 py-20 text-center">
                      <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">No sessions recorded yet</p>
                    </td>
                  </tr>
                )}
              </tbody>
